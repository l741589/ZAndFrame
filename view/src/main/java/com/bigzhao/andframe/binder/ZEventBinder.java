...
public class ZEventBinder extends ZBaseBinder implements EventType {
    ...
    public static void bindEvent(Object holder,EventListenerStub stb){
        ...
        List<Method>ms = Ref.getAllMethods(cls, "java.", "android.", "com.bigzhao.andframe.");
        for (Method m:ms){
    ...
    private static View getContentView(Object holder){
        if (holder instanceof IViewHolder) return ((IViewHolder)holder).getContentView();
        return ZUtil.getContentView(holder);
    }

    private static Object bindViewEvent(Object holder,Method m,int id) {
        if (id == 0) return null;
        View target = null;
        View v = getContentView(holder);
        if (v != null) {
